...
class Swagger {
  ...
  build() {
        ...
        if (routerObject) {
          ...
          for (const tag of tags) {
            ...
            if (title === 'description' && description.length) {
              const types = description.match(/#(parameters|responses|consumes|produces|tag)/g);
              if (!types) continue;
    ...
    const bundleFileString = fs.readFileSync(swaggerUiBundlePath).toString().replace('https://petstore.swagger.io/v2/swagger.json', `${this.app.config.static.prefix}/swagger.json`);
