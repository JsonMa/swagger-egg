...
class Swagger {
      ...
      for (const comment of comments) {
        ...
        if (routerObject) {
          ...
          for (const tag of tags) {
            ...
            // 添加 接口描述功能
            if(title === 'summary' && description.length) {
              this.getSummary(path,method,description)
            }
   ...
   * @param {*} description 
   */
   getSummary(path,method,description) {
    this.swaggerObject.paths[path][method].description = description;
  }
