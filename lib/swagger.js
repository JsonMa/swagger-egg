...
class Swagger {
  ...
  build() {
    ...
    this.swaggerObject = utils.prepareSwaggerObject(
      this.app,
      utils.prepareDefaultOptions(this.config)
    );
      ...
      for (const comment of comments) {
        ...
        tags.forEach((tag, index) => {
          if (tag.title === 'function') {
            assert(
              !functionIndex,
              `Duplex function tag of ${JSON.stringify(tag)}`
            );
            functionIndex = index;
          }
        });
        ...
        if (routerObject) {
          ...
          for (const tag of tags) {
            ...
            if (title === 'description' && description.length) {
              const types = description.match(
                /#(parameters|responses|consumes|produces|tags)/g
              );
              if (!types) continue;
    ...
    const bundleFileString = utils.prepareSwaggerUi(
      fs.readFileSync(swaggerUiBundlePath).toString(),
  ...
  getCommonParser(path, method, commomParams, type) {
    ...
    const trimParams = params.map((param) => {
      ...
      if (type === 'tags') {
        assert(
          this.tags.includes(trimParams),
          `${param} tag is not defined in swagger config`
        );
      }
      return trimParams;
    });
