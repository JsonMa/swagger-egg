...
class RouterLoader {
  ...
  getCommonRouter(fileString) {
    const routers = fileString.match(
      /(head|options|get|put|post|patch|delete|del|redirect)\((\s|\S)*?\)/g
    );
    if (routers) {
      routers.forEach((router) => {
        const methodResult = router.match(
          /^(head|options|get|put|post|patch|delete|del|redirect)/g
        );
        ...
        const path = `${this.prefix}${pathNameArray
          .pop()
          .replace(/\'|`/g, '')}`;
        const name = pathNameArray.length
          ? pathNameArray.pop().replace(/\'|`/g, '')
          : null;
  ...
  getRouter() {
    ...
    if (dependencies) {
      dependencies
        ...
        .filter(
          (dependency) =>
            dependency.includes('/router/') || dependency.includes('/routers/')
        )
        .forEach((dependencyPath) => {
          const dependencyFileString = fs
            .readFileSync(
              path.join(path.dirname(this.filePath), `${dependencyPath}.${this.config.ts ? 'ts' : 'js'}`)
            )
            .toString();
